...
void test_case_sm3(zmcrypto::sdk* _sdk)
{
    std::vector<key_val_vec> test_vec;
    if (!read_vector_data(TEST_VECTOR_PATH "sm3.txt", test_vec)){
	...
	for (size_t i = 0; i < test_vec.size(); i++){
        ...
        #if defined ZMCRYPTO_ALGO_SHA1
        {
            ...
            uint8_t* output = new uint8_t[_sdk->zm_sm3_digest_size()];
            ...
            if (digest == std::string((char*)output, _sdk->zm_sm3_digest_size())){
        ...
        #if defined TEST_FOR_CRYPTOPP
        {
            CryptoPP::HashTransformation* HashPtr = new CryptoPP::SM3();
            for (uint32_t i = 0; i < loop;i ++){
...
void test_speed_sm3(zmcrypto::sdk* _sdk)
{
    #if defined ZMCRYPTO_ALGO_SM3
    {
        ...
        while (true)
        {
            sdk.zm_sm3_update (ctx, (uint8_t*)msg, mlen);
            dsize += mlen;
        ...
        format_output("sm3 by zmcrypto|%s/s\n", bytes_to_human_readable_format((uint64_t)(rate * 1000000)).c_str());

        sdk.zm_sm3_final (ctx, output);
        sdk.zm_sm3_free (ctx);
    ...
    #if defined TEST_FOR_CRYPTOPP && defined TEST_FOR_CRYPTOPP_SPEED
    {
        ...
        format_output("sm3 by Crypto++|%s/s\n", bytes_to_human_readable_format((uint64_t)(rate * 1000000)).c_str());
    ...
    #if defined TEST_FOR_OPENSSL_SPEED
    {
        ...
        format_output("sm3 by OpenSSL|%s/s\n", bytes_to_human_readable_format((uint64_t)(rate * 1000000)).c_str());
