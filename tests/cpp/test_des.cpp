...
namespace{
        ...
        zmcrypto::sdk g_des_sdk;
        void*   cb_des_new            (void) { return g_des_sdk.zm_des_new(); }
        void    cb_des_free           (void* ctx) { g_des_sdk.zm_des_free((des_ctx*)ctx); }
        void    cb_des_init           (void* ctx) { g_des_sdk.zm_des_init((des_ctx*)ctx); }
        int32_t cb_des_block_size     (void) { return g_des_sdk.zm_des_block_size(); }
        int32_t cb_des_ksize_min      (void) { return g_des_sdk.zm_des_ksize_min(); }
        int32_t cb_des_ksize_max      (void) { return g_des_sdk.zm_des_ksize_max(); }
        int32_t cb_des_ksize_multiple (void) { return g_des_sdk.zm_des_ksize_multiple(); }
        int32_t cb_des_set_ekey       (void* ctx, uint8_t* key, uint32_t ksize) { return g_des_sdk.zm_des_set_ekey((des_ctx*)ctx, key, ksize); }
        int32_t cb_des_set_dkey       (void* ctx, uint8_t* key, uint32_t ksize) { return g_des_sdk.zm_des_set_dkey((des_ctx*)ctx, key, ksize); }
        void    cb_des_enc_block      (void* ctx, uint8_t* plaintext, uint8_t* ciphertext) {g_des_sdk.zm_des_enc_block((des_ctx*)ctx, plaintext, ciphertext); }
        void    cb_des_dec_block      (void* ctx, uint8_t* ciphertext, uint8_t* plaintext) {g_des_sdk.zm_des_dec_block((des_ctx*)ctx, ciphertext, plaintext); }
    #endif
}

void test_case_des_ecb(zmcrypto::sdk* _sdk)
{
    std::vector<key_val_vec> test_vec;
    if (!read_vector_data(TEST_VECTOR_PATH "des.txt", test_vec)){
    ...
    for (size_t i = 0; i < test_vec.size(); i++){
        ...
        #if defined ZMCRYPTO_ALGO_ECB && defined ZMCRYPTO_ALGO_DES
        ...
        /* Encryption */
        {
            ...
            int32_t block_size = cb_des_block_size ();
            int32_t ksize_min = cb_des_ksize_min ();
            int32_t ksize_max = cb_des_ksize_max ();
            int32_t ksize_multiple = cb_des_ksize_multiple ();

            if (algorithm == "des-ecb"){
                _sdk->zm_ecb_init(ctx, cb_des_new, cb_des_free, cb_des_init, cb_des_block_size, cb_des_ksize_min, cb_des_ksize_max, cb_des_ksize_multiple, cb_des_set_ekey, cb_des_set_dkey, cb_des_enc_block, cb_des_dec_block);
            }else{
        ...
        /* Encryption */
        {
            ...
            CryptoPP::BlockCipher* pCipher = new CryptoPP::DES::Encryption();
            ...
            int block_size = CryptoPP::DES::BLOCKSIZE;
        ...
        /* Decryption */
        {
            ...
            CryptoPP::BlockCipher* pCipher = new CryptoPP::DES::Decryption();
            pCipher->SetKey((CryptoPP::byte*)(key.c_str()), (uint32_t)key.length());
...
void test_case_des_cbc(zmcrypto::sdk* _sdk)
{
#if 1
    std::vector<key_val_vec> test_vec;
    if (!read_vector_data(TEST_VECTOR_PATH "des.txt", test_vec)){
    ...
    for (size_t i = 0; i < test_vec.size(); i++){
        ...
        #if defined ZMCRYPTO_ALGO_CBC && defined ZMCRYPTO_ALGO_DES
        ...
        /* Encryption */
        {
            ...
            if (algorithm == "des-cbc"){
                ...
                _sdk->zm_cbc_init(ctx, cb_des_new, cb_des_free, cb_des_init, cb_des_block_size, cb_des_ksize_min, cb_des_ksize_max, cb_des_ksize_multiple, cb_des_set_ekey, cb_des_set_dkey, cb_des_enc_block, cb_des_dec_block);
            }else{
                _sdk->zm_cbc_free (ctx);
                continue;
            }
...
void test_case_des_cfb(zmcrypto::sdk* _sdk)
{
#if 1
    std::vector<key_val_vec> test_vec;
    if (!read_vector_data(TEST_VECTOR_PATH "des.txt", test_vec)){
    ...
    for (size_t i = 0; i < test_vec.size(); i++){
        ...
        #if defined ZMCRYPTO_ALGO_CFB && defined ZMCRYPTO_ALGO_DES
        ...
        /* Encryption */
        {
            ...
            if (algorithm == "des-cfb"){
                if (iv.empty()){
                    printf("iv is empty\n");
                    _sdk->zm_cfb_free (ctx);
                    return;
                }
                _sdk->zm_cfb_init(ctx, cb_des_new, cb_des_free, cb_des_init, cb_des_block_size, cb_des_ksize_min, cb_des_ksize_max, cb_des_ksize_multiple, cb_des_set_ekey, /*cb_des_set_dkey, */cb_des_enc_block/*, cb_des_dec_block*/);
            }else{
                _sdk->zm_cfb_free (ctx);
                continue;
            }
...
void test_case_des_ofb(zmcrypto::sdk* _sdk)
{
#if 1
    std::vector<key_val_vec> test_vec;
    if (!read_vector_data(TEST_VECTOR_PATH "des.txt", test_vec)){
    ...
    for (size_t i = 0; i < test_vec.size(); i++){
        ...
        #if defined ZMCRYPTO_ALGO_OFB && defined ZMCRYPTO_ALGO_DES
        ...
        /* Encryption */
        {
            ...
            if (algorithm == "des-ofb"){
                ...
                _sdk->zm_ofb_init(ctx, cb_des_new, cb_des_free, cb_des_init, cb_des_block_size, cb_des_ksize_min, cb_des_ksize_max, cb_des_ksize_multiple, cb_des_set_ekey, /*cb_des_set_dkey, */cb_des_enc_block/*, cb_des_dec_block*/);
            }else{
                _sdk->zm_ofb_free (ctx);
                continue;
            }
...
void test_case_des_ctr(zmcrypto::sdk* _sdk)
{
#if 1
    std::vector<key_val_vec> test_vec;
    if (!read_vector_data(TEST_VECTOR_PATH "des.txt", test_vec)){
    ...
    for (size_t i = 0; i < test_vec.size(); i++){
        ...
        #if defined ZMCRYPTO_ALGO_CTR && defined ZMCRYPTO_ALGO_DES
        ...
        /* Encryption */
        {
            ...
            if (algorithm == "des-ctr"){
                if (iv.empty()){
                    printf("iv is empty\n");
                    _sdk->zm_ctr_free (ctx);
                    return;
                }
                _sdk->zm_ctr_init(ctx, cb_des_new, cb_des_free, cb_des_init, cb_des_block_size, cb_des_ksize_min, cb_des_ksize_max, cb_des_ksize_multiple, cb_des_set_ekey, /*cb_des_set_dkey, */cb_des_enc_block/*, cb_des_dec_block*/);
            }else{
                _sdk->zm_ctr_free (ctx);
                continue;
            }
...
void test_speed_des(zmcrypto::sdk* _sdk)
{
    ...
    uint8_t ct[8] = { 0 };
        ...
        {
            CONTEXT_TYPE_PTR(des) ctx = _sdk->zm_des_new();
            _sdk->zm_des_set_ekey(ctx, key, key_sizes[i]);
            ...
            while (true)
            {
                _sdk->zm_des_enc_block(ctx, pt, ct);
                end = get_timestamp_us();
                dsize += _sdk->zm_des_block_size();
        ...
        {
            CONTEXT_TYPE_PTR(des) ctx = _sdk->zm_des_new();
            _sdk->zm_des_set_dkey(ctx, key, key_sizes[i]);
            ...
            while (true)
            {
                _sdk->zm_des_dec_block(ctx, pt, ct);
                end = get_timestamp_us();
                dsize += _sdk->zm_des_block_size();
                if (end - start >= TEST_TOTAL_SEC * 1000000){
                    break;
...
void test_info_des(zmcrypto::sdk* _sdk)
{
    
#if defined ZMCRYPTO_ALGO_DES
    int32_t blocksize = _sdk->zm_des_block_size();
    int32_t min = _sdk->zm_des_ksize_min();
    int32_t max = _sdk->zm_des_ksize_max();
    int32_t mutiple = _sdk->zm_des_ksize_multiple();

    printf ("des block size: %d, min key size: %d, max key size: %d, key size multiple: %d\n",
