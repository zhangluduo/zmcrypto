...
{
    if (!ctx) return -1;
    else if (keylen > 0 && !key) return -3;

   int i;

   // [BEGIN] Add by Zhang Luduo, 2020-06-10
   if (outlen != 20 && outlen != 32 && outlen != 48 && outlen != 64)
...
void blake2b_final(blake2b_ctx *ctx, void *out)
{
   ...
   for (i = 0; i < ctx->outlen; i++) {
       ((uint8_t *) out)[i] =
           (uint8_t)((ctx->h[i >> 3] >> (8 * (i & 7))) & 0xFF);
   }
}

// Convenience function for all-in-one computation.

int blake2b(void *out, int outlen,
...
int blake2b_hmac_starts(blake2b_ctx *ctx, int outlen, const void *key, int keylen)
{
    ...
    if( keylen > _BLAKE2B_BLOCK )
    {
		int ret = blake2b(sum, outlen, NULL, 0, key, keylen);
		if (ret != 0)
		{
			delete[] sum;
			sum = NULL;
    ...
    memset( ctx->ipad, 0x36, _BLAKE2B_BLOCK );
    memset( ctx->opad, 0x5C, _BLAKE2B_BLOCK );
	...
	if (blake2b_init(ctx, outlen, NULL, 0) != 0)
		return -1;

	blake2b_update(ctx, ctx->ipad, _BLAKE2B_BLOCK);
	return 0;
}
...
{
    unsigned char* tmpbuf = new unsigned char[(int)(ctx->outlen)];

    blake2b_final( ctx, tmpbuf );
    blake2b_init( ctx, (int)(ctx->outlen), NULL, 0 );
    blake2b_update( ctx, ctx->opad, _BLAKE2B_BLOCK );
    blake2b_update( ctx, tmpbuf, (int)(ctx->outlen) );
    blake2b_final( ctx, out );
