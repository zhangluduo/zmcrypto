...
*/
void cast5_done(cast5_key *skey)
{
}
                    ...
                    unsigned char *output )
{
    ...
    if( length % 8 )
        return 1;//( POLARSSL_ERR_DES_INVALID_INPUT_LENGTH );

    if( mode == CAST5_ENCRYPT )
    {
        while( length > 0 )
        {
            for( i = 0; i < 8; i++ )
			...
			cast5_ecb_encrypt(output, output, ctx);
            memcpy( iv, output, 8 );
    ...
    else /* DES_DECRYPT */
    {
        ...
        {
            memcpy( temp, input, 8 );
			cast5_ecb_decrypt(input, output, ctx);
                       ...
                       unsigned char *output )
{
    ...
    {
        if( n == 0 ) {
			cast5_ecb_encrypt(nonce_counter, temp, ctx);
