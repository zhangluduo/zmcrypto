...
*/
void kseed_done(kseed_key *skey)
{
}
                    ...
                    unsigned char *output )
{
    ...
    if( length % 16 )
        return 0;//( POLARSSL_ERR_DES_INVALID_INPUT_LENGTH );

    if( mode == SEED_ENCRYPT )
    {
        while( length > 0 )
        {
            for( i = 0; i < 16; i++ )
			...
			kseed_ecb_encrypt( output, output, ctx );
            memcpy( iv, output, 16 );
    ...
    else /* DECRYPT */
    {
        while( length > 0 )
        {
			...
			kseed_ecb_decrypt( input, output, ctx );
                       ...
                       unsigned char *output )
{
    ...
    {
        if( n == 0 ) {
            kseed_ecb_encrypt( nonce_counter, temp, ctx );
