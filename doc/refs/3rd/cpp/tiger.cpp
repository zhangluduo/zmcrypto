...
{
    ctx->state[0] = CONST64(0x0123456789ABCDEF);
    ctx->state[1] = CONST64(0xFEDCBA9876543210);
    ctx->state[2] = CONST64(0xF096A5B4C3B2E187);
    ctx->curlen = 0;
    ctx->length = 0;
    return CRYPT_OK;
}

static int tiger_compress(tiger_context *ctx, unsigned char *buf)
{
    ulong64 a, b, c, x[8];
    unsigned long i;
    ...
    /* store state */
    ctx->state[0] = a ^ ctx->state[0];
    ctx->state[1] = b - ctx->state[1];
    ctx->state[2] = c + ctx->state[2];
...
int tiger_process (tiger_context * ctx, const unsigned char *in, unsigned long inlen)
{
    ...
    while (inlen > 0) {
        if (ctx->curlen == 0 && inlen >= _TIGER_BLOCK_SIZE) {
           if ((err = tiger_compress (ctx, (unsigned char *)in)) != CRYPT_OK) {
              return err;
           }
           ctx->length += _TIGER_BLOCK_SIZE * 8;
           in += _TIGER_BLOCK_SIZE;
           inlen -= _TIGER_BLOCK_SIZE;
        } else {
           n = MIN(inlen, (_TIGER_BLOCK_SIZE - ctx->curlen));
           memcpy(ctx->buf + ctx->curlen, in, (size_t)n);
           ctx->curlen += n;
           in += n;
           inlen -= n;
           if (ctx->curlen == _TIGER_BLOCK_SIZE) {
              if ((err = tiger_compress (ctx, ctx->buf)) != CRYPT_OK) {
                 return err;
              }
              ctx->length += 8*_TIGER_BLOCK_SIZE;
              ctx->curlen = 0;
           }
       }
    }
...
int tiger_done(tiger_context * ctx, unsigned char *out)
{
    ...
    /* append the '1' bit */
    ctx->buf[ctx->curlen++] = (unsigned char)0x01;
    ...
    /* copy output */
    STORE64L(ctx->state[0], &out[0]);
    STORE64L(ctx->state[1], &out[8]);
    STORE64L(ctx->state[2], &out[16]);
...
{
	memset( ctx->ipad, 0x36, _TIGER_BLOCK_SIZE );
	memset( ctx->opad, 0x5c, _TIGER_BLOCK_SIZE );

	unsigned char key_temp[_TIGER_BLOCK_SIZE];

	if (keylen > _TIGER_BLOCK_SIZE)
	{
		tiger(key, keylen, key_temp);
		keylen = _TIGER_DIGEST_SIZE;
	}
	else
	{
	...
	tiger_init(ctx);
	tiger_process(ctx, ctx->ipad, _TIGER_BLOCK_SIZE);
...
{
	unsigned char temp[_TIGER_DIGEST_SIZE];
	tiger_done(ctx, temp);

	tiger_init(ctx);
	tiger_process(ctx, ctx->opad, _TIGER_BLOCK_SIZE);
	tiger_process(ctx, temp, _TIGER_DIGEST_SIZE);
	tiger_done(ctx, out);
