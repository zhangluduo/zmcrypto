...
#define _lrotr(x,n)   (((x) >> ((int)(n))) | ((x) << (32 - (int)(n))))
#define _lrotl(x,n)   (((x) << ((int)(n))) | ((x) >> (32 - (int)(n))))
...
// 2014-11-13
int serpent_crypt_cbc( serpent_key *ctx,
                    int mode,
                    int length,
                    unsigned char iv[16],
                    ...
                    unsigned char *output )
{
    ...
    if( length % 16 )
        return( POLARSSL_ERR_SERPENT_INVALID_INPUT_LENGTH );

    if( mode == SERPENT_DECRYPT )
    {
        while( length > 0 )
        {
            memcpy( temp, input, 16 );
			serpent_ecb_decrypt(input, output, ctx);
    ...
    else
    {
        while( length > 0 )
        {
			...
			serpent_ecb_encrypt(output, output, ctx);
            memcpy( iv, output, 16 );
	...
	unsigned char stream_block[16], const unsigned char *input, unsigned char *output )
{
    ...
    {
        if( n == 0 ) {
			serpent_ecb_encrypt(nonce_counter, stream_block, ctx);
