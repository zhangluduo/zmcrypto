...
#include "sm4.h"
    ...
    */
    static uint32_t sm4CalciRK(uint32_t ka)
    {
        ...
        rk = bb ^ (ROTL32(bb, 13)) ^ (ROTL32(bb, 23));
        return rk;
    }
    ...
    */
    static uint32_t sm4Lt(uint32_t ka)
    {
        ...
        b[3] = sm4Sbox(a[3]);
        GET_UINT32_BE(bb, b, 0);
        c = bb ^ (ROTL32(bb, 2)) ^ (ROTL32(bb, 10)) ^ (ROTL32(bb, 18)) ^ (ROTL32(bb, 24));
        return c;
    }
    ...
    *
    */
    static void sm4_one_round (uint32_t sk[32], uint8_t input[16], uint8_t output[16])
    {
        uint32_t i = 0;
        uint32_t ulbuf[36];

        zmcrypto_memset(ulbuf, 0, sizeof(ulbuf));
