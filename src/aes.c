...
#include "aes.h"
    ...
    */
    ...
    struct aes_ctx* aes_new (void)
    {
        struct aes_ctx* ctx = (struct aes_ctx*)zmcrypto_malloc(sizeof(struct aes_ctx));
        zmcrypto_memset(ctx, 0, sizeof(struct aes_ctx));
        return ctx;
    }
    ...
    int32_t aes_set_ekey (struct aes_ctx* ctx, uint8_t* key, uint32_t ksize)
    {
        uint32_t i;
        uint32_t *RK;
    ...
    zmerror aes_set_dkey (struct aes_ctx* ctx, uint8_t* key, uint32_t ksize)
    {
        struct aes_ctx cty;
        uint32_t i, j;        
        uint32_t *RK;
        ...
        *RK++ = *SK++;
        ...
        zmcrypto_memset( &cty, 0, sizeof( struct  aes_ctx ) );
        return ZMCRYPTO_ERR_SUCCESSED;
    }
