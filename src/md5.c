...
#include "md5.h"
    ...
    struct md5_ctx* md5_new (void)
    {
        struct md5_ctx* ctx = (struct md5_ctx*)zmcrypto_malloc(sizeof(struct md5_ctx));
        zmcrypto_memset(ctx, 0, sizeof(struct md5_ctx));
        return ctx;
    }
    ...
    void md5_starts (struct md5_ctx* ctx) 
    { 
        ctx->total[0] = 0;
        ctx->total[1] = 0;
    ...
    void md5_update (struct md5_ctx* ctx, uint8_t* data, uint32_t dsize)
    { 
        uint32_t fill;
        uint32_t left;
