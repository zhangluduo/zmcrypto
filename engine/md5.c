...
void md5_process (md5_ctx *ctx, const uint8_t data[64])
{
...
md5_ctx* md5_new (void)
{
    ...
    md5_ctx* ctx = (md5_ctx*)zmcrypto_malloc(sizeof(md5_ctx));
    zmcrypto_memset(ctx, 0, sizeof(md5_ctx));
    return ctx;
}
...
void md5_update (md5_ctx* ctx, uint8_t* data, uint32_t dlen)
{ 
    printf("%s:%d %s\n", __FILE__, __LINE__, __FUNCTION__);
    uint32_t fill;
    uint32_t left;
...
void md5_final (md5_ctx* ctx, uint8_t* output)
{
    printf("%s:%d %s\n", __FILE__, __LINE__, __FUNCTION__);
    uint32_t last, padn;
    uint32_t high, low;
